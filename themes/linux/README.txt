...
Create a config.rb Compass project file in the root of the theme folder. You will need to add the following to make the theme work correctly with Compass:
...
If your theme is using Zen Grids, swap the require 'susy' with:
...
The theme uses SASS and Compass to build the CSS, then is compiled into the /css/style.css file. The SASS files are organized in a file structure that is based on overall site structure, not specific components. The structure is as follows in the /css/sass folder:
...
* **_font-awesome.scss** - This is a `@mixin` file containing icons that can be added to elements. To use this on any CSS selector, use the `@include icon-name;` on the selector, and the icon will be added, as a font based icon. To use this set of icons, visit http://fortawesome.github.com/Font-Awesome/. Download the library, and put the font files in the theme /fonts folder. You can also see the icon classes and example on the Font Awesome site. The names are included beside the icon samples.
...
* **_reset.scss** - This is a CSS reset, based on Normalize.css, and there is no need to edit.
